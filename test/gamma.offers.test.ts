...
import { deployNofFixture, getOnePackData, PackDataType } from './common'
...
describe('NoF - Gamma Offers Tests', function () {
  ...
  it('createOffer should revert when cardNumber in wantedCardNumbers', async () => {
    ...
    const packData: PackDataType = await getOnePackData(gammaPacks, gammaCards, address0)
    await expect(
      gammaOffers.createOffer(packData[0][0], [packData[0][0],2,24,4,5,6,7,8])
  ...
  it('createOffer should revert when user does not have cardNumber', async () => {
    ...
    const packData: PackDataType = await getOnePackData(gammaPacks, gammaCards, address0)
    await expect(
      gammaOffers.createOffer(140, [packData[0][0],2,24,4,5,6,7,8])
  ...
  it('User could create offer by its cardNumber', async () => {
    const { gammaPacks, gammaCards, gammaOffers, address0 } = await loadFixture(deployNofFixture)
    const packData: PackDataType = await getOnePackData(gammaPacks, gammaCards, address0)
    await gammaOffers.createOffer(packData[0][0], [1,2,24,4,5,6,7,8])
  ...
  it('should not allow a user to create an offer for an existing cardNumber', async () => {
    const { gammaPacks, gammaCards, gammaOffers, address0 } = await loadFixture(deployNofFixture)
    const packData1: PackDataType = await getOnePackData(gammaPacks, gammaCards, address0)
    const packData2: PackDataType = await getOnePackData(gammaPacks, gammaCards, address0)
    await gammaOffers.createOffer(packData1[0][0], [1,2,24,4,5,6,7,8])
    const offers = await gammaOffers.getOffers()
    await expect(offers.length).not.equal(0)
    await expect(offers[offers.length-1].owner).to.be.equal(address0.address)
    await expect(
      gammaOffers.createOffer(packData2[0][0], [1,2,24,4,5,6,7,8])
  ...
  it('should retrieve an offer using getOffersByCardNumber', async () => {
    ...
    const packData: PackDataType = await getOnePackData(gammaPacks, gammaCards, address0)
    await gammaOffers.createOffer(packData[0][0], [1,2,24,4,5,6,7,8])
    const offers = await gammaOffers.getOffersByCardNumber(packData[0][0])
    await expect(offers.length).not.equal(0)
    await expect(offers[0].offerId).not.equal(0)
    await expect(offers[0].cardNumber).to.be.equal(packData[0][0])
  })

  it('should retrieve an offer using getOfferByUserAndCardNumber', async () => {
    const { gammaPacks, gammaCards, gammaOffers, address0 } = await loadFixture(deployNofFixture)
    const packData: PackDataType = await getOnePackData(gammaPacks, gammaCards, address0)
    const cardNumber = packData[0][0]
  ...
  it('should delete an offer by user and cardNumber', async () => {
    const { gammaPacks, gammaCards, gammaOffers, address0 } = await loadFixture(deployNofFixture)
    const packData: PackDataType = await getOnePackData(gammaPacks, gammaCards, address0)
    const card1 = packData[0][0];
    const card2 = packData[0][1];
    const card3 = packData[0][2];
    ...
    await gammaOffers.createOffer(packData[0][0], [1,2,24,4,5,6,7,8])
    await gammaOffers.createOffer(packData[0][1], [1,2,24,4,5,6,7,8])
  ...
  it('should delete all offers', async () => {
    ...
    const offersCounter = await gammaOffers.getOffersCounter();
    const offersByUserCounter = await gammaOffers.getOffersByUserCounter(address0.address);
    const offersByCardNumberCounter1 = await gammaOffers.getOffersByCardNumberCounter(packData[0][0]);
    const offersByCardNumberCounter2 = await gammaOffers.getOffersByCardNumberCounter(packData[0][1]);
